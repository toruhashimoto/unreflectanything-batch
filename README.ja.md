...
| `--workers N` | `auto` | 並列ワーカープロセス数（`auto` / 整数、`1`=逐次）。1枚ごとの処理は CPU/I-O 律速で GPU はほぼ遊ぶため、多コア機ではこれが最大のレバー。luma は強くスケール（実測 ~13倍）。AI はワーカー毎にモデル再ロードのため（16並列ロードはディスク/CUDA を圧迫＝実測で約3倍*遅い*）、auto は上限8（実測 ~2.9倍）・小バッチは自動縮小。非常に大きいバッチのみ明示指定で増やす。GUI は逐次 |
