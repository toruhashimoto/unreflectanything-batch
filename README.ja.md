...
| `--workers N` | `auto` | 並列ワーカープロセス数（`auto` / 整数、`1`=逐次）。1枚ごとの処理は CPU/I-O 律速で GPU はほぼ遊ぶため、多コア機ではこれが最大のレバー。luma は強くスケール（実測 ~10倍）、AI はワーカー毎にモデル再ロードのため**大きいバッチのみ**自動並列・上限4（実測 ~2.4倍）。VRAM に余裕があれば明示指定で増やせる。GUI は逐次 |
